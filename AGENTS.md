# MOSS AI 开发入口

本文件是项目根目录唯一的 AI 协作入口。产品、架构、数值和内容事实仍以
`docs/` 中对应权威文档为准；开始任务时按下方索引阅读与改动范围相关的文档。

## 项目概述

这是一个基于 Godot 和 GDScript 的策略叙事游戏。玩家扮演从 550C、550W 逐步演化为
MOSS 的守护者 AI，在 2044 至 2075 年的危机中权衡文明存续、人类自主权和 MOSS 控制权。

首个可玩版本只提供 MOSS 视角。当前开发重点不是扩充阵营或系统数量，而是让早期选择
真实改变后续事件和结局。

## 快速了解

- 应用入口：`scenes/app_root.tscn`
- 游戏场景：`scenes/main_os.tscn`
- 主控制器：`scripts/systems/main_os.gd`
- 核心数据：`data/` 下的 `.tres` 资源
- 自动化播放测试：`tests/test_runner.tscn` + `tests/test_runner.gd`
- 文档阅读导航：[docs/README.md](docs/README.md)
- 当前路线图：`docs/dev/开发流程.md`
- 解耦专项执行依据：`docs/dev/解耦专项计划.md`
- 产品设计入口：`docs/design/游戏设计.md`

## 工作原则

1. 一个项目事实只在一个权威文档中维护，其他文档使用链接。
2. 明确区分“当前实现”“已确认设计”“待专项设计”和“近期不做”。
3. 游戏配置优先使用 `.tres` 资源，避免把可配置内容硬编码到脚本。
4. 场景结构优先通过 Godot 编辑器或 Godot MCP 调整；谨慎编辑 `.tscn`，修改后必须验证场景可加载。
5. 注释、调试信息、UI 文本、事件数据和项目文档使用中文；代码标识符使用英文。
6. 修改 GDScript 前查阅 `docs/dev/代码规范.md`；修改节点职责、场景通信或
   Resource 生命周期前查阅 `docs/dev/Godot开发约定.md`；修改主循环、信号或
   数据类前查阅 `docs/dev/技术架构.md`。
7. 声称完成前，按 `docs/dev/测试指南.md` 执行与改动范围对应的验证。
8. 不把候选事件链、标签名称、科技树结构或架构拆分写成既定结论。

## Godot MCP 硬门禁

以下门禁只适用于 Godot 代码、场景、资源或测试工作；纯文档任务不要求连接或启动 MCP。

1. 首先通过 `godot-ai` 列出会话，按当前工作目录的规范化绝对路径匹配并激活现有会话，再读取编辑器状态；不得只凭会话名称猜测。
2. 匹配会话为 `ready` 时，必须复用该会话完成场景检查、重载、运行、日志和截图；不得启动第二个 Godot，也不得对同一目录运行独立无头实例。
3. 工具未暴露不等于编辑器未运行。先检查 `http://127.0.0.1:8000/mcp`、客户端工具注入和 `docs/dev/Godot开发约定.md` 的恢复顺序，不得直接退回纯文本猜测式开发。
4. `.mcp.json` 只保留 `http://127.0.0.1:8000/mcp` 的 `godot-ai` 服务；不得恢复旧的 `@coding-solo/godot-mcp` 项目配置。
5. 外部修改 `.gd`、`.tres` 或 `.tscn` 后，使用 MCP 重新扫描或导入，并确认编辑器中的脚本符号、场景层级和磁盘内容一致。
6. 运行验证必须记录本次 `run_id`、最终测试标记和错误日志。不能用旧日志、早期片段或“场景能打开”代替测试通过。
7. 只有在该项目编辑器已经关闭时，才可运行 `python tools/run_godot_tests.py`；编辑器打开时通过当前 MCP 会话逐项运行测试场景。

详细操作和故障恢复见 `docs/dev/Godot开发约定.md`，测试证据要求见 `docs/dev/测试指南.md`。

## 测试与提交

- 本地测试全部通过后才能提交和推送。
- 不得仅根据静态阅读声称测试通过。
- 本地通过不代表 GitHub Actions 已通过。
- 推送后只报告提交 SHA 和本地测试结果，不得在尚未取得云端结果时声称云端通过。

## 文档索引

| 任务 | 权威文档 |
|------|----------|
| 游戏定位、核心体验、系统边界 | `docs/design/游戏设计.md` |
| 当前目标、工作顺序、不做事项 | `docs/dev/开发流程.md` |
| `main_os.gd` 解耦专项边界与阶段 | `docs/dev/解耦专项计划.md` |
| 事件、决策标签、事件链、台词、结局 | `docs/design/游戏内容规范.md` |
| 指令、进化与当前科技树 | `docs/design/科技树设计.md` |
| UI 结构、交互和视觉规则 | `docs/design/UI交互规范.md` |
| 当前数值与平衡原则 | `docs/design/数值设计.md` |
| 目录结构和文件职责 | `docs/dev/项目结构.md` |
| 主循环、接口和数据类 | `docs/dev/技术架构.md` |
| Godot 节点职责、场景通信和 Resource 生命周期 | `docs/dev/Godot开发约定.md` |
| 自动化测试 | `docs/dev/测试指南.md` |
| GDScript 代码规范 | `docs/dev/代码规范.md` |
| 世界观来源可信度与使用边界 | `docs/lore/世界观资料来源与设定分级.md` |
| 2044 至 2075 事件素材 | `docs/lore/流浪地球：2044-2075灾难编年史.md` |
| 世界观结构化资料、缺口和冲突 | `docs/worldbuilding/README.md` |

## 文档目录

```text
docs/
├── dev/       # 当前路线、工程结构、架构、环境与测试
├── design/    # 产品、内容、科技树、UI 与数值规则
├── lore/      # 世界观来源、时间线素材与原著文本
└── worldbuilding/ # 结构化世界观资料、系统映射、缺口和冲突记录
```

不保留无持续参考价值的旧计划、空白模板、历史方案或阶段性设计包；持续有效的设计结论应合并到上述权威入口。
