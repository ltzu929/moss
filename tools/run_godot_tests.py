...
TEST_SCENES = [
    ...
    ("tests/strategic_director_test.tscn", "headless"),
    ("tests/random_event_director_test.tscn", "headless"),
...
def run_godot(godot, args, project_root):
    command = [godot, *args]
    try:
...
def main():
    ...
    exit_code = run_godot(godot, ["--headless", "--editor", "--path", ".", "--quit"], project_root)
    if exit_code != 0:
    ...
    for scene, mode in TEST_SCENES:
        ...
        if command_args is None:
            ...
            continue
        exit_code = run_godot(godot, [*command_args, scene], project_root)
        if exit_code != 0:
