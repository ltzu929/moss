...
- 核心历史已落地 `decision.core_2044_automation_access`、`decision.core_2053_population_vs_infrastructure` 和 `decision.core_2058_crisis_authority`：前两者分别进入后续危机与结局，2058 标签由 2065、2075 和结局读取。
- 2075 年根据控制权、秩序、希望和科技核心判定结局；结局文本会读取 2044、2053、2058 核心标签和少量代表轻量事件状态作为“历史回顾”，这些历史事实不改变结局类型判定。
...
结局类型判定仍只读取科技核心、控制权、平均秩序和平均希望。结局正文已读取 2044、2053、2058 核心标签，以及地下城迁移、丫丫样本访问、数字生命泄露、热屏蔽短缺和最终授权会议等代表轻量状态，用于解释历史事实，不新增第五类结局；其中 2075 主事件会读取 2053 与 2058 标签补充长期治理回声。
...
- 事件选择已经通过轻量 `event_state.*` 形成长期因果，并落地 2044、2053、2058 三个核心历史标签；2065、2070 核心标签和独立分支触发仍未实现。
- 结局已经引用 2044、2053、2058 核心标签和少量代表早期选择，但其余核心标签与更多路线组合仍需补充。
