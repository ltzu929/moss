...
- 处理 2044-2075 六个主事件和 17 个中型事件。
- 在固定研究年份获得协议点并激活科技节点。
- 中型事件写入轻量 `event_state.*`，主事件读取这些状态生成历史回声；2058、2065、2070 已有代表状态接入主事件选项代价。
- 核心历史已落地 `decision.core_2044_automation_access` 和 `decision.core_2053_population_vs_infrastructure`：前者由 2058、2065 和结局读取，后者由 2075 和结局读取。
- 2075 年根据控制权、秩序、希望和科技核心判定结局；结局文本会读取 2044、2053 核心标签和少量代表轻量事件状态作为“历史回顾”，这些历史事实不改变结局类型判定。
...
结局类型判定仍只读取科技核心、控制权、平均秩序和平均希望。结局正文已读取 2044、2053 核心标签，以及地下城迁移、丫丫样本访问、数字生命泄露、热屏蔽短缺和最终授权会议等代表轻量状态，用于解释历史事实，不新增第五类结局；其中 2075 主事件也会读取 2053 标签补充长期治理回声。

## 组织和派系

- 已实现组织：联合政府仍作为世界观组织存在，但不再是独立区域卡或事件地区。
- 已实现地区/板块：亚洲、欧洲、非洲、大洋洲、北美、南美，见 `data/sector_*.tres`；欧洲承接原联合政府与俄罗斯板块的游戏目标。
...
- 事件选择已经通过轻量 `event_state.*` 形成长期因果，并落地 2044、2053 两个核心历史标签；2058、2065、2070 核心标签和独立分支触发仍未实现。
- 结局已经引用 2044、2053 核心标签和少量代表早期选择，但其余核心标签与更多路线组合仍需补充。
