...
2026-09-07 文档核对：按磁盘资源核对事件、科技和局势目录，并核对调速、存档的实现入口；本次未运行游戏测试，运行验收以[测试指南](../dev/测试指南.md)和对应开发记录为准。本文是阅读概览，完整规则分别链接到设计文档，不作为第二份数值规范。
...
- 在固定研究年份获得协议点并激活科技节点。
- 处理持续影响资源与地区的随机局势。现有题材和与固定事件的边界见[游戏设计](../design/游戏设计.md)；这些局势为游戏原创桥接。
- 使用主页面、全局调速和本地存档。存档由 `scripts/systems/save_service.gd` 维护，调速入口为 `MainOS.set_time_speed()`；交互规则见 [UI 交互规范](../design/UI交互规范.md)。
...
科技节点与路线规则以[科技树设计](../design/科技树设计.md)为准，真实内容来自 `data/technology/*.tres`。原作只提供 550 系列、设施与技术背景，以下节点名称和部署含义属于游戏桥接。
...
- 来源规则与逐年素材：`docs/lore/世界观资料来源与设定分级.md`、`docs/lore/流浪地球：2044-2075灾难编年史.md`。
- 产品和内容入口：`docs/design/游戏设计.md`、`docs/design/游戏内容规范.md`、`docs/design/科技树设计.md`。
- 结构化资料库：[世界观入口](README.md)；网络资料及访问边界：[网络资料核对](sources/网络资料核对.md)。
...
- 事件与科技进一步关联的候选题材见[系统映射](游戏系统世界观映射.md)；尚未新增数字生命科技路线或第五类结局。
